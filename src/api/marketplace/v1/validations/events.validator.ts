...
// Check if description exists and if so, check if it is a string
const AddEvent_DescriptionValidation = check('description').custom((value) => {
  /**
   * Check if the description is not empty
   * If it is empty, return true
   * If it is not empty, check if it is a string
...
const AddEvent_EndIsAfterStart = check('end').custom((value, { req }) => {
  if (value < req.body.start) {
    throw new Error("The 'end' parameter date must be after the 'start' parameter date.");
  }
  return true;
});
...
const UpdateEvent_DescriptionValidation = check('description').custom((value) => {
  if (value !== '' && value !== null && value !== undefined) {
    /**
