...
export default class AdminCaregiversController {
  // db
   ...
   * @description
   */
  static async adminCreateCaregiver(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      try {
        ...
        healthUnit = await AdminCaregiversController.HealthUnitsDAO.retrieve(healthUnitId);
      } catch (error: any) {
        switch (error.type) {
          case 'NOT_FOUND': {
      ...
      try {
        existingcaregiver = await AdminCaregiversController.CaregiversDAO.queryOne({
      ...
      try {
        ...
        newcaregiver = await AdminCaregiversController.CaregiversDAO.create(caregiver);
      } catch (error: any) {
        // If there was an error creating the caregiver in the database, delete the caregiver from Cognito.
        const deletecaregiver = await AdminCaregiversController.CognitoService.adminDeleteUser(
          reqCaregiver.email
        );
  ...
  static async retrieveCaregiver(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      try {
        // Get caregiver by id
        caregiver = await AdminCaregiversController.CaregiversDAO.retrieve(caregiverId);
      } catch (error: any) {
        switch (error.type) {
          case 'NOT_FOUND':
  ...
  static async updateCaregiver(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      try {
        caregiverExists = await AdminCaregiversController.CaregiversDAO.retrieve(caregiverId);
        ...
        if (caregiverExists) {
          ...
          updatedcaregiver = await AdminCaregiversController.CaregiversDAO.update(caregiver!);
        }
      } catch (error: any) {
        switch (error.type) {
          case 'NOT_FOUND':
  ...
  static async deleteCaregiver(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const orders = (
        await AdminCaregiversController.HomeCareOrdersDAO.queryList({
      ...
      if (caregiver) {
        try {
          const deletedCaregiver = await AdminCaregiversController.CaregiversDAO.delete(
            caregiverId
          );
      ...
      if (caregiver?.cognito_id) {
        try {
          await AdminCaregiversController.CognitoService.adminDeleteUser(caregiver.cognito_id);
        } catch (error: any) {
          switch (error.type) {
            default:
              return next(new HTTPError._500(error.message));
  ...
  static async listCaregivers(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      try {
        caregivers = (
          await AdminCaregiversController.CaregiversDAO.queryList({
