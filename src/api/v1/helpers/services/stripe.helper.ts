...
export default class StripeHelper {
  ...
  async getConnectedAccountActiveClients(accountId: string) {
  ...
  async getConnectedAccountCurrentMRR(accountId: string) {
     ...
     * This should be done by getting the invoices from the previous month. We expand the charge object so that we only account for paid invoices.
  ...
  async getConnectAccountTotalRevenueByMonth(accountId: string) {
    try {
   ...
   */
  async calculateApplicationFeeWithPromotionCode(amount: number, promotionCodeId: string): Promise<number> {
    ...
    let applicationAmount = amount * (parseFloat(STRIPE_APPLICATION_FEE) / 100) - parseFloat(coupon.amount_off) / 100;
     ...
     */
    let newApplicationFee = (applicationAmount * 100 / (amount - parseFloat(coupon.amount_off) / 100)).toFixed(2);
  
    if (parseFloat(newApplicationFee) > 100 || parseFloat(newApplicationFee) < 0) {
