...
export default class CustomersController {
  ...
  static FavouritesDAO = new FavouritesDAO();
  ...
  static async createFavourite(
    ...
    try {
      ...
      let Favourite: IFavouriteDocument;
      ...
      try {
        // Get the Favourite from MongoDB.
        Favourite = await CustomersController.FavouritesDAO.retrieve(FavouriteId);
      } catch (error: any) {
        switch (error.type) {
          case 'NOT_FOUND': {
      ...
      sanitizedReqCustomer.health_unit = Favourite._id;
  ...
  static async listFavourites(
    ...
    try {
      ...
      const FavouriteId = user.health_unit._id.toString();
  ...
  static async retrieveFavourite(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
  ...
  static async updateFavourite(
    ...
    try {
      ...
      if (customerExists?.health_unit?.toString() !== FavouriteId) {
        return next(new HTTPError._403('You are not authorized to perform this action.'));
      }
