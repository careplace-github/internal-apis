...
router
  .route('/users/orders')
  .get(AuthenticationGuard, AccessGuard('marketplace'), OrdersController.listOrdersByUser);
router
  .route('/users/orders/:id')
  .get(AuthenticationGuard, AccessGuard('marketplace'), OrdersController.retrieve);
